...
public class GeneratorServiceImpl implements GeneratorService {
    ...
    public void verify(String tableName) throws Exception {
        try {
            ...
            if (columnIdinfo == null) {
                String sql = "ALTER TABLE " + tableName + " ADD COLUMN id bigint(20) NOT NULL COMMENT 'ID' FIRST";
                Query query = em.createNativeQuery(sql);
                query.executeUpdate();
                query = em.createNativeQuery("ALTER TABLE " + tableName + " ADD PRIMARY KEY (id)");
                query.executeUpdate();
            }
            else {
                if (!columnIdinfo.getIsNullable().equals("NO") || !columnIdinfo.getColumnType().equals("bigint") || !columnIdinfo.getColumnComment().equals("ID") || !columnIdinfo.getColumnKey().equals("PRI") || !columnIdinfo.getExtra().equals("")) {
                    String sql = "ALTER TABLE " + tableName + " MODIFY COLUMN id bigint(20) NOT NULL COMMENT 'ID' FIRST";
                    Query query = em.createNativeQuery(sql);
                    query.executeUpdate();
                }
            }
            ...
            if (columnPublishedinfo == null) {
                String sql = "ALTER TABLE " + tableName + " ADD COLUMN published timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '创建日期' AFTER id";
                Query query = em.createNativeQuery(sql);
                query.executeUpdate();
            }
            else {
                if (!columnPublishedinfo.getIsNullable().equals("NO") || !columnPublishedinfo.getColumnType().equals("timestamp") || !columnPublishedinfo.getColumnComment().equals("创建日期") || !columnPublishedinfo.getColumnKey().equals("") || !columnPublishedinfo.getExtra().equals("")) {
                    String sql = "ALTER TABLE " + tableName + " MODIFY COLUMN published timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '创建日期' AFTER id";
                    Query query = em.createNativeQuery(sql);
                    query.executeUpdate();
                }
            }
            ...
            if (columnUpdatedinfo == null) {
                String sql = "ALTER TABLE " + tableName + " ADD COLUMN updated timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '更新日期' AFTER published";
                Query query = em.createNativeQuery(sql);
                query.executeUpdate();
            }
            else {
                if (!columnUpdatedinfo.getIsNullable().equals("NO") || !columnUpdatedinfo.getColumnType().equals("timestamp") || !columnUpdatedinfo.getColumnComment().equals("更新日期") || !columnUpdatedinfo.getColumnKey().equals("") || !columnUpdatedinfo.getExtra().equals("")) {
                    String sql = "ALTER TABLE " + tableName + " MODIFY COLUMN updated timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '更新日期' AFTER published";
                    Query query = em.createNativeQuery(sql);
                    query.executeUpdate();
                }
            }
            ...
            if (columnAuthorinfo == null) {
                String sql = "ALTER TABLE " + tableName + " ADD COLUMN author varchar(32) NOT NULL COMMENT '创建者' AFTER updated";
                Query query = em.createNativeQuery(sql);
                query.executeUpdate();
            }
            else {
    ...
    private void veriyPrimaryKey(String tablename) throws Exception {
        String sql = "select column_name from information_schema.key_column_usage where table_name = ? and column_name = 'id' and constraint_name = 'PRIMARY'";
        Query query = em.createNativeQuery(sql);
        query.setParameter(1, tablename);
        List result = query.getResultList();
        if (result.size() == 0 ) {
        ...
        sql = "select column_name from information_schema.key_column_usage where table_name = ? and column_name != 'id' and constraint_name = 'PRIMARY'";
